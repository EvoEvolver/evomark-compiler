...
import { evomark_tokenizer, get_close_tag, get_closed_tag, get_open_tag, get_tag_pair, push_warning, token, tokeniz_rule_func, tokenize_box } from "../tokenize";
...
function tokenize(root: parse_node, tokens: token[], tokener: evomark_tokenizer){
    ...
    for (let child of root.children) {
        ...
        if (child.type == "func_body") {
            ...
            tokenize_box(child, tokens, tokener)
        }
    }
    tokens.push(close)
}
...
export function figure(core: evomark_core){
    ...
    core.tokenizer.add_func_rule(new tokeniz_rule_func("figure", tokenize))
